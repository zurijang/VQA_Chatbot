...
def webhook(request):
    ...
    if ((is_json_key_present(telegram_update['message'], 'document')) and (telegram_update['message']['document']['mime_type'] == "image/jpeg" \
        ...
        if not is_json_key_present(telegram_update['message'], 'document') and not is_json_key_present(telegram_update['message'], 'video') \
            and not is_json_key_present(telegram_update['message'], 'photo'): # TEXT만 받도록, IMAGE 외의 documnet (파일) 거름
            print('TEXT')
            ...
            print('압축된 파일 전송')
            text = "압축되지 않은 이미지를 전송해주세요.\n첨부파일→파일→갤러리(압축 없이 사진 보내기)"
            bot.send_message(chat_id=chatID, text=text)
